        ...
        public async Task<IViewComponentResult> InvokeAsync(Models.Component component)
        {
            ...
            var values = _context.PropertyValues.Include(i => i.Entity).Include(i => i.Property).ThenInclude(t => t.DataSourceProperty).ThenInclude(v => v.PropertyValues).Where(x => x.EntityId == entityId && _context.PropertyValues.Where(
                  p => (datatable.Columns.Any(c => c.FilterOperator != Models.FilterOperator.None) ? (datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterOperator == Models.FilterOperator.Equals ?
                      (p.Value == string.Format(datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterValue, datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).Property.DataSourceProperty.Id)) : (datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterOperator == Models.FilterOperator.Contains ? (p.Value.Contains(string.Format(datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterValue, pId))) : ((datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterOperator == Models.FilterOperator.DoesNotContain ? (!p.Value.Contains(string.Format(datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterValue, pId))) : ((datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterOperator == Models.FilterOperator.LessThan ? (p.Value.CompareTo(string.Format(datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterValue, pId)) > 0) : ((datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterOperator == Models.FilterOperator.LessThanOrEquals ? (p.Value.CompareTo(string.Format(datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterValue, pId)) >= 0) : ((datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterOperator == Models.FilterOperator.GreaterThan ? (p.Value.CompareTo(string.Format(datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterValue, pId)) < 0) : ((datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterOperator == Models.FilterOperator.GreaterThanOrEquals ? (p.Value.CompareTo(string.Format(datatable.Columns.FirstOrDefault(c => c.PropertyId == p.PropertyId).FilterValue, pId)) <= 0) : false)))))))))))) : true)).Any(f => f.RowId == x.RowId)).OrderBy(r => r.RowId).Take(datatable.Top).ToList();
